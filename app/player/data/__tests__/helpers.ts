...
export function expectNoteType(
  ...
  if (item.data.type !== type) {
    throw new Error(
      `Expected note type '${type}' but got '${item.data.type}'`
    );
  }
  return true;
}

export function expectLane(item: { type: "note"; data: NoteData }, lane: number): boolean {
  if (item.data.lane !== lane) {
    throw new Error(
      `Expected lane ${lane} but got lane ${item.data.lane}`
    );
  }
  return true;
}
