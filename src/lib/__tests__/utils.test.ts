...
describe("Test Helpers", () => {
  ...
  describe("expectValidChart", () => {
    ...
    it("should throw for empty array", () => {
      expect(() => expectValidChart([])).toThrow(
        "Chart result must not be empty",
      );
    });
  });
  ...
  describe("snapshotPath", () => {
    it("should generate consistent snapshot identifiers", () => {
      ...
      expect(snapshotPath("Complex Chart Parsing!")).toBe(
        "complex-chart-parsing",
      );
    });
  });
...
describe("Test Fixtures", () => {
  ...
  describe("BPM_CHANGE", () => {
    it("should parse multiple time signatures", () => {
      ...
      const timeSignatures = result.filter(
